...
# Generate a perfect random maze via randomized DFS
...
# Compute solution path via BFS for validation
...
# Initialize maze and solution once
...
def move():
    now = time.time()
    # Limit to one move per second
    if now - session.get('last_move', 0) < 1.0:
        return jsonify(success=False, message='Too fast! Wait 1s.'), 429
